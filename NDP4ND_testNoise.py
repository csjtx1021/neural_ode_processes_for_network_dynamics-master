...
parser.add_argument('--noise', type=float, default=0.01, metavar='S',
                    help='noise (default: 0.01)')
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, x):  # How to use t?
        ...
        out_dynamics = out_F + scatter_sum(out_G, col, dim=1, dim_size=x.size(1))
...
def make_batch(data, batch_size, is_shuffle=True, bound_t_context=None, is_test=False, is_shuffle_target=True,
    ...
    while start_idx < len(tasks_data_index_shuffle):
        ...
        for task_i_idx in tasks_data_index_shuffle[start_idx:start_idx + batch_size]:
            ...
            for idx in range(len(contexts_index)):
                ...
                contexts_batch_x_self.append(torch.from_numpy(np.concatenate(
                    [contexts_point_i['x_self_w_noise'],
                     np.zeros((contexts_point_i['x_self_w_noise'].shape[0], max_x_dim - contexts_point_i['x_self_w_noise'].shape[-1]))],
                    axis=-1)).float())
                contexts_batch_mask.append(torch.from_numpy(
                    contexts_point_mask[idx].reshape(-1, 1) * get_mask_dim(contexts_point_i['x_self_w_noise'].shape[-1],
                                                                           max_x_dim).reshape(1, -1)).long())
                contexts_batch_adj.append(torch.from_numpy(contexts_point_i['adj']).long())
            ...
            for targets_point_idx in targets_index:
                ...
                targets_batch_mask.append(torch.from_numpy(
                    targets_point_i['mask'].reshape(-1, 1) * get_mask_dim(targets_point_i['x_self'].shape[-1],
                                                                          max_x_dim).reshape(1, -1)).long())
                targets_batch_adj.append(torch.from_numpy(targets_point_i['adj']).long())
        ...
        batch_data = {"adj": torch.from_numpy(batch_adj).long(),
...
def fit_score_net(score_net_path, paths, dataset):
    ...
    gen_batch = make_batch(dataset.data, 8, is_shuffle=False)
    # make train datas
    Rs = []
    ys = []
    count_ = 0
    ...
    print('make model')
    score_net = ScoreNet(hidden_dim, num_heads=4)
    ...
    np.savetxt('score_net_pre_on_trainset_%s.txt'%len(paths), torch.softmax(pre_on_trainset, dim=-1).detach().cpu().numpy())
...
def test_score_net(score_net, pre_models, adj, x0, task_info, x_context):
    Rs_ = []
    for idx in range(len(pre_models)):
        res = pre_models[idx](adj, x0, task_info, x_context,
                              (x_context[0], None, x_context[2], x_context[3], x_context[4], x_context[5]),
...
def test_net_dynamic_with_2nd_phase(paths=None, score_net_path=None, batch_data=None, saved_results=None, add_str="", is_plot=True):
    ...
    score_net = ScoreNet(hidden_dim, num_heads=4).to(device)
    score_net.load_state_dict(torch.load(score_net_path))
    score_net.eval()
    ...
    weights = test_score_net(score_net, pre_models, adj, x0, task_info, x_context)
    ...
    std_ys = torch.sqrt(torch.sum((mean_ys_ ** 2 + std_ys_ ** 2) * weights.view(-1,1,1,1,1,1), dim=0) - mean_ys ** 2) ## [num_sampling,  #nodes, 1, #steps, d]
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        for dynamics_name_i, topo_type_i in dynamics_topo_names:
            ...
            test_data_set_path = 'data/DynamicsData/test_data_on_dynamics_%s_topo_%s_dataset_x%s_numgraph1_timestep100_seed666_num_nodes=%s_noise=%s_split_train_and_test%s.pickle' % \
                                 (dynamics_name_i, topo_type_i, x_dim_, test_N, args.noise, add_str)
            ...
            if args.constraint_state:
                ...
                saved_fname = "./results/saved_test_results_%s_MLloss%s_deter%s_uncer%s_%s_%s_x%s_numgraph%s_timestep%s" % \
                              (model.name, model.use_ML_loss, model.is_determinate, model.is_uncertainty,
                               dynamics_name_i_add,
                               topo_type_i_add, x_dim_, num_graphs,
                               time_steps) + "_epoch%s_bound_t_context%s_seed%s_num_nodes%s_noise%s" % \
                              (num_epochs, bound_t_context, seed, test_N, args.noise)
